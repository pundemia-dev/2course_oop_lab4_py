...
class Oval:
    def __init__(self, p, color, canvas):
        self.r = Point(35, 15)
        self.canvas = canvas
        self.p = self.constrain_oval_move(p)
        self.color = color
        self.selected = False
    ...
    def resize(self, lp, np):
        if self.selected:
            ...
            self.r = self.constrain_oval_resize(self.r.inc(Point(offset_x, offset_y)))

    def move(self, offset:Point):
        if self.selected:
            self.p = self.constrain_oval_move(self.p.inc(offset))
            
    def fill(self, color):
        if self.selected:
            self.color = color

    def constrain_oval_move(self, value:Point):
        return Point(
    ...
    def constrain_oval_resize(self, r):
